...
export function Logo({ className, height = 12, title = 'PROJECTUS' }: { className?: string; height?: number; title?: string }) {
  return (
    <span
      ...
      dangerouslySetInnerHTML={{ __html: logoUrl.replace(/<svg([^>]*)>/, `<svg$1 height="${height}" width="auto" style="display:block">`) }}
    />
  )
}
