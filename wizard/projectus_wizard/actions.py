...
async def build_server_macos(root: Path, log: LogFn, i18n: I18n) -> None:
    await log(i18n.t("server.build_frontend"))
    await run_command(["pnpm", "--dir", str(root), "build"], cwd=root, log=log)
    await log(i18n.t("server.build_release"))
    await run_command(
        ["cargo", "build", "--release", "-p", "projectus-server", "--manifest-path", str(root / "Cargo.toml")],
        cwd=root,
        log=log,
    )
    await log(i18n.t("server.ready", path=macos.release_server_binary(root)))
...
async def build_all_macos(root: Path, log: LogFn, i18n: I18n) -> None:
    ...
    await restart_server_macos(root, log, i18n)
...
async def report_environment(root: Path, log: LogFn, i18n: I18n) -> None:
    ...
    await log(
        i18n.t(
            "status.launch_agent",
            state=i18n.t("value.installed" if state.launch_agent_installed else "value.not_installed"),
            path=state.launch_agent_path,
        )
    )
    await log(
        i18n.t(
    ...
    if state.app_installed:
        await log(i18n.t("status.app_installed", state=i18n.t("value.yes"), path=state.app_path))
        await log(i18n.t("status.app_version", version=state.app_version or i18n.t("value.unavailable")))
    else:
        await log(i18n.t("status.app_installed", state=i18n.t("value.no"), path=state.app_path))
